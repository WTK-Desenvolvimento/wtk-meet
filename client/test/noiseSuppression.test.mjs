...
test('chaves desconhecidas são descartadas na leitura e na escrita', () => {
  ...
  assert.deepEqual(writeAudioPreferences(storage, { rumor: true }), { noiseSuppression: false });
...
test('modo: sem constraint nativa, mas com AudioWorklet, o fallback assume', () => {
  assert.equal(
    decideCapabilityMode({ supportedConstraints: { echoCancellation: true }, audioWorkletSupported: true }),
    MODE.WORKLET,
  );
});
...
const rmsOf = (buffer, from = 0, to = buffer.length) => {
  let sum = 0;
...
function process(state, signal, enabled = true) {
...
const whiteNoise = (samples, amplitude, seed) => {
