 ...
 * a validação no navegador fica com `e2e/run.ts` (checagens M1–M6).
 *
...
function once(socket: Socket, event: string, timeout = EVENT_TIMEOUT): Promise<Record<string, unknown>> {
...
async function enterEmptyRoom(roomId: string, displayName?: string) {
...
async function requestEntry(roomId: string, displayName?: unknown) {
  const socket = await connect();
  socket.emit('join-request', { roomId, displayName });
  return socket;
}
...
test('aprovar admite quem esperava e avisa o resto da sala', async () => {
  ...
  assert.deepEqual(
    // O cast: `once` devolve o payload cru do socket, e o que se afirma é
    // justamente o conteúdo da lista.
    (approval.members as { displayName: string }[]).map((m) => m.displayName),
    ['Alice'],
