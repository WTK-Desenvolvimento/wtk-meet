 ...
 * (mesmo padrão de `musicMeshRouting.test.mjs`): o que se afirma é que o
...
const { default: PeerAudio } = await import('../src/components/PeerAudio.jsx');
const { default: VideoTile } = await import('../src/components/VideoTile.jsx');
...
const peerAudioSource = componentSource('PeerAudio.jsx');
const remoteMusicSource = componentSource('RemoteMusicAudio.jsx');
...
test('PeerAudio monta um <audio> por peer com stream, e nenhum é muted', () => {
  ...
  const html = renderToStaticMarkup(
    createElement(PeerAudio, { participants, sinkId: 'spk-b', unlockNonce: 0 }),
  );
...
test('a transição de um peer que já saiu não o ressuscita', () => {
  ...
  // fantasma na grade.
  const before = new Map([['p1', { displayName: 'Alice' }]]);
...
test('o mesh real entrega a transição a quem passar onPeerStateChange', async () => {
  ...
  try {
    ...
    let participants = new Map([['p1', { displayName: 'Alice' }]]);
    const [pc] = created;
