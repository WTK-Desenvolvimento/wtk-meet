...
function installFakeBrowser() {
  ...
  // Os casts do ambiente: cada dublê implementa o que `audioLevels` chama, e
  // nenhum deles cabe na interface completa do navegador.
...
/** Stream dublê: o monitor só pergunta se há track de áudio. */
const fakeStream = {
...
test('close() encerra o AudioContext e solta os analisadores', async () => {
  ...
  const monitor = new AudioLevelMonitor({ onUpdate: () => {} });
  monitor.attach('alice', fakeStream);
  // O `!`: o `attach` acima abriu o contexto.
