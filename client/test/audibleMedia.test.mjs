 ...
 * `useEffect`) ou o dispatcher raso de `settingsNoiseToggle.test.mjs` (que
 ...
 */
function renderWithEffects(Component, initialProps) {
  const hooks = [];
  const effects = [];
  let cursor = 0;
  let pending = [];
  ...
  function flush() {
    ...
    for (const slot of slots) {
      ...
      effects[slot].cleanup = effects[slot].create() || undefined;
    }
  }
 ...
 */
function fakeElement({ setSinkId = () => Promise.resolve(), play = () => Promise.resolve() } = {}) {
  const element = {
    tagName: 'AUDIO',
    srcObject: null,
    sinkCalls: [],
...
/** Duplo de `MediaStream`: só o alvo de eventos que o hook escuta. */
function fakeStream(id = 's1') {
...
const mount = (element, options) => {
  ...
  return renderWithEffects((props) => useAudibleMedia(ref, props), options);
};
...
test('a rejeição de setSinkId vira onSinkError, e não unhandledrejection', async () => {
  ...
  const seen = [];
  mount(element, { stream: fakeStream(), sinkId: 'spk-morto', onSinkError: (err) => seen.push(err) });
