 ...
 *
 * `youtubePlayer.test.mjs` já cobre as duas pontas isoladas: o envelope emite
 ...
 *
 * 1. **Dublê de DOM e de `window.YT`**, como em `youtubePlayer.test.mjs`. O
...
async function waitFor(predicate, { timeout = 2_500, label = 'condição' } = {}) {
...
function ytEntry({ id, sourceRef, title, lamport = 1, addedBy = OTHER }) {
  return { id, kind: 'youtube', title, sourceRef, durationSec: 240, addedBy, addedByName: 'Outro', lamport };
}

function snapshotMessage({ entries, playback }) {
 ...
 */
async function mountRoom({ ownerId, entries, entryId, version = 1 }) {
  const sent = [];
  const props = {
    meshRef: { current: { sendMusicMessage: (payload) => sent.push(payload), setMusicTrack: async () => {} } },
  ...
  const room = renderHook(useMusicRoom, props);
  // O host é do React na vida real; aqui o nó falso faz o mesmo papel.
  room.result.youtubeHostRef.current = makeNode('div');
  ...
  return {
    ...
    playersSince: (mark) => YT.players.length - mark,
    /** Quantas vezes **este vídeo** subiu de novo desde o marco. */
    reloadsSince: (mark, videoId) => YT.players.slice(mark).filter((player) => player.videoId === videoId).length,
    mark: () => YT.players.length,
    unmount: () => room.unmount(),
  };
}

const removals = (sent) => sent.filter((message) => message.type === 'music-queue-remove');
