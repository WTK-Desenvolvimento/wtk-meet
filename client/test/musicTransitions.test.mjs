...
const NAMES = { 'peer-alice': 'Alice', 'peer-bob': 'Bob', 'peer-carol': 'Carol' };

function track({ id, addedBy, lamport, kind = 'url', sourceRef }) {
...
function sessionWith(entries, playback = {}) {
...
function createRoom(entries) {
  ...
  return {
     ...
     */
    advance(finishedEntryId, reason, { present = PEERS } = {}) {
      const plans = present.map((selfId) => ({
        ...
        plan: planAdvance({
          ...
          presentIds: present,
          selfId,
          delivery: (entry) => (entry.kind === 'youtube' ? 'local' : 'stream'),
        }),
      }));
