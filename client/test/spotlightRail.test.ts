 ...
 * puro (`spotlightLayout.test.ts`) não consegue provar sozinho: que a lista
 * renderizada contém todo mundo, e que a miniatura de tela é um controle de
 * verdade para quem usa teclado ou leitor de tela.
 *
 * O E2E (`e2e/run.ts`, checagens C5–C11) é quem prova isso no navegador, mas o
 ...
 * JSX é transformado pelo esbuild que vem com o Vite (ver `tools/tsLoader.mjs`).
 *
 ...
 * o E2E; a aritmética por trás delas está em `spotlightLayout.test.ts`.
 */
...
const { default: ThumbnailRail } = await import('../src/components/ThumbnailRail.js');
const { default: SpotlightStage } = await import('../src/components/SpotlightStage.js');

import type { ComponentProps } from 'react';
import type { Tile } from '../src/components/VideoTile.js';

const camera = (key: string, extra: Partial<Tile> = {}): Tile => ({ key, audioId: key, label: key, ...extra });
const screenOf = (key: string, owner: string): Tile => ({
  key,
  screenId: key,
  owner,
  label: `${owner} — tela`,
...
const renderRail = (props: Partial<ComponentProps<typeof ThumbnailRail>> & { items: Tile[] }) =>
  renderToStaticMarkup(
    createElement(ThumbnailRail, { audioLevels: {}, onSelectScreen: () => {}, ...props }),
  );

/** Rótulos visíveis, na ordem em que aparecem na coluna. */
const labels = (html: string) => [...html.matchAll(/<span class="video-label">([^<]*)</g)].map((m) => m[1]);

/** Cada `<button>` da coluna, com os atributos que importam para o teclado. */
function selectButtons(html: string) {
...
test('aria-pressed marca a tela em destaque e só ela', () => {
  ...
  const pressedStates = (spotlightId: string) =>
