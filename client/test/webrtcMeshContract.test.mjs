...
class FakePeerConnection {
  ...
  addTransceiver(kind, { direction } = {}) {
    const t = {
      ...
      kind,
      direction,
      sender: { track: null, replaceTrack: async function (track) { this.track = track; }, getParameters: null },
    };
 ...
 */
function makeMesh({ iceServers = TURN, localStream = null, selfId = 'aaa-eu', ...rest } = {}) {
...
test('closeAll solta todos os handlers da conexão', async () => {
  ...
  try {
    ...
    for (const nome of [
      ...
      assert.equal(pc[nome], null, `${nome} continuou ligado depois do fechamento`);
    }
  } finally {
    h.restore();
  }
