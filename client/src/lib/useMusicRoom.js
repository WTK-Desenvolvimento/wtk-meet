...
export function useMusicRoom({ meshRef, participants, getSelfId, displayName, pushToast }) {
  ...
  useEffect(() => {
    ...
    publishTimerRef.current = setInterval(() => {
      ...
      // Erro em tratamento ou retentativa em curso: o player que falhou responde
      // posição congelada e estado parado. `loading` só cobre o `load()` — a
      // janela entre o erro e a recarga fica descoberta sem esta marca, e o que
      // se publicaria nela é `playing: false` como verdade para a sala inteira.
