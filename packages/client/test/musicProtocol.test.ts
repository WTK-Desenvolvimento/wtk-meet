...
import type { SanitizedMusicMessage } from '../src/lib/musicProtocol.js';

/**
 * Estreita o resultado para a variante que o caso está exercitando.
 *
 * O `throw` é a mesma reprovação que a asserção daria — só que dizendo qual
 * variante veio no lugar. O `as` da última linha é o único do arquivo: o
 * compilador não estreita uma união por comparação com um genérico, mesmo
 * depois do `if` que faz exatamente essa comparação.
 */
function comoTipo<T extends SanitizedMusicMessage['type']>(
  msg: SanitizedMusicMessage | null,
  tipo: T,
): Extract<SanitizedMusicMessage, { type: T }> {
  if (!msg) throw new Error(`esperava ${tipo}, veio null`);
  if (msg.type !== tipo) throw new Error(`esperava ${tipo}, veio ${msg.type}`);
  return msg as Extract<SanitizedMusicMessage, { type: T }>;
}
