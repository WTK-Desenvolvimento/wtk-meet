...
import type { PeerRecord } from '../src/lib/webrtcMesh.js';
import type { DevicePreferences } from '../src/lib/devices.js';

/** `localStorage` de mentira, só com o que `readPreferences` consome. */
function storageWith(value?: Partial<DevicePreferences> | Record<string, unknown>) {
...
function pedeVideo(plan: ReturnType<typeof initialMediaPlan>) {
...
/**
 * Os dublês implementam **o que o mesh chama**. Os `as unknown as` estão na
 * fronteira (`globalThis` e as opções do mesh), cada um comentado.
 */
...
class FakePeerConnection {
  ...
  transceivers: { mid: string | null; kind: string; direction?: string; sender: { replaceTrack: (t: unknown) => Promise<void> } }[];
...
/** O que o mesh lê de uma track local. */
interface FakeTrack {
  kind: string;
  id: string;
  stop(): void;
...
const fakeTrack = (kind: string): FakeTrack => ({
  kind,
  id: `${kind}-1`,
  stop() {},
  addEventListener() {},
...
async function meshEntrando({ comCamera }: { comCamera: boolean }) {
  ...
  const mesh = new WebRTCMesh({
    ...
    iceServers: [],
    localStream: new FakeMediaStream(tracks) as unknown as MediaStream,
    getSelfId: () => 'eu',
    getRoomKey: () => null,
    getIceServers: async () => [],
  });
  await mesh.addPeer('quem-ja-estava');
  // O `!`: `addPeer` acabou de registrar o par.
...
test('o `state` enviado quando o canal abre carrega o cameraOff: true da entrada', async () => {
  ...
  const state = canalDe(rec).sent.find((m) => m.type === 'state');
