...
import type { GridLayout, GridLayoutInput } from '../src/lib/gridLayout.js';
...
const layout = (box: Caixa, count: number, extra: GridLayoutInput = {}) =>
  computeGridLayout({ ...box, count, ...extra });

/** A grade inteira cabe na caixa, nos dois eixos. */
function assertFits(box: Caixa, result: GridLayout, label: string) {
...
function bestPossibleTileWidth(box: Caixa, count: number, gap = GRID_GAP) {
  let best = 0;
