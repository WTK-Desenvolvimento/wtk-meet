...
import type { PeerRecord, SignalPayload, WebRTCMeshOptions } from '../src/lib/webrtcMesh.js';
 ...
 * inteira — é o que os mantém legíveis. O preço são os `as unknown as` na
 * fronteira (`installGlobals` e as opções do mesh), cada um comentado onde está.
 */
interface FakeTrack {
  kind: string;
  id: string;
...
/** Um transceiver dublê: só `mid`, `direction`, `kind` e o sender. */
interface FakeTransceiver {
  mid: string | null;
  direction?: string;
  kind: string;
  sender: { replaceTrack: (track: unknown) => Promise<void>; getParameters: (() => unknown) | null };
}

/** Uma linha do `trace`: o nome da operação e o que ela recebeu. */
...
class FakePeerConnection {
  ...
  /** Ligado por um caso só: o navegador que recusa reconfigurar em voo. */
  setConfigurationThrows?: boolean;
  ...
  emitIce(state: string) {
    this.iceConnectionState = state;
    this.oniceconnectionstatechange?.();
  }

  emitSignaling(state: string) {
...
function installGlobals() {
  // A fronteira do dublê: os dois `as unknown as` existem porque o dublê
  // implementa o que o mesh chama, e não a interface inteira do navegador —
  // que traria `generateCertificate`, `addTrack`, `clone` e mais uma dúzia de
  // membros que nenhum teste exercita.
...
/**
 * O caminho inverso: o que o mesh guarda é o dublê, e é ele que o teste
 * inspeciona. Um lugar só para o cast, em vez de um por asserção.
 */
const canalDe = (rec: PeerRecord | undefined): FakeDataChannel =>
...
async function meshWithPeer({
  peerId = 'peer-b',
  selfId = 'peer-a',
  iceQueue = [TURN] as IceServer[][],
  ...overrides
}: {
  peerId?: string;
  selfId?: string;
  iceQueue?: IceServer[][];
} & Partial<WebRTCMeshOptions> = {}) {
  installGlobals();
  FakePeerConnection.reset();

  const events: {
    peerState: [string, string][];
    remoteScreen: [string, MediaStream | null][];
    signals: [string, SignalPayload][];
  } = { peerState: [], remoteScreen: [], signals: [] };
  const iceCalls: { force?: boolean }[] = [];
  const pending = [...iceQueue];

  const mesh = new WebRTCMesh({
    signaling: { sendSignal: (id, data) => events.signals.push([id, data]) },
    iceServers: [],
    localStream: comoNavegador<MediaStream>(
      new FakeMediaStream([fakeTrack('audio', 'mic'), fakeTrack('video', 'cam')]),
    ),
    getSelfId: () => selfId,
    getRoomKey: () => null,
  ...
  // O `!` e o cast: `addPeer` acabou de registrar o par, e a conexão que ele
  // guarda é o dublê — quem a construiu foi o `installGlobals` acima.
  const rec = mesh.peers.get(peerId)!;
  const pc = rec.pc as unknown as FakePeerConnection;
  return { mesh, rec, pc, events, iceCalls };
}
...
test('duas chamadas concorrentes de addPeer constroem UMA conexão (A13, R1)', async () => {
  ...
  const mesh = new WebRTCMesh({
    ...
    iceServers: [],
    localStream: comoNavegador<MediaStream>(new FakeMediaStream([])),
    getSelfId: () => 'peer-a',
...
test('as tentativas respeitam o backoff, param no teto e o contador zera (A18)', async (t) => {
  ...
  const origError = console.error;
  const erros: string[] = [];
  console.error = (...a: unknown[]) => erros.push(a.join(' '));
  try {
...
test('state-request responde ao remetente e só a ele, coalescido (A20, R6)', async (t) => {
  ...
  canalDe(rec).sent.length = 0;
  canalDe(outro).sent.length = 0;

  canalDe(rec).onmessage!({ data: JSON.stringify({ type: 'state-request' }) });
  assert.deepEqual(canalDe(rec).types(), ['state'], 'responde com o estado corrente');
  assert.deepEqual(canalDe(outro).types(), [], 'responder com broadcast seriam N mensagens, não 1');

  canalDe(rec).onmessage!({ data: JSON.stringify({ type: 'state-request' }) });
  assert.deepEqual(canalDe(rec).types(), ['state'], 'pedido repetido em janela curta não gera resposta');
});
...
test('o canal que abre pede o estado do outro lado, além de anunciar o seu', async () => {
  ...
  assert.deepEqual(canalDe(rec).types(), ['state', 'state-request']);
});
...
test('ao voltar de uma recuperação, o par reafirma estado e snapshot (A21)', async (t) => {
  ...
  // O snapshot mínimo que o mesh reafirma: `playback` nulo é o estado de sala
  // sem faixa, e é o que este caso exercita.
...
test('conectar sem ter passado por recuperação NÃO reanuncia nada', async (t) => {
  ...
  assert.deepEqual(canalDe(rec).types(), [], 'reafirmar em toda transição é amplificação');
});
...
test('tipo desconhecido no canal é ignorado sem erro no console (A22)', async () => {
  ...
  const { errors, warns } = await captureConsole(() => {
    canalDe(rec).onmessage!({ data: JSON.stringify({ type: 'invencao-do-futuro', x: 1 }) });
    canalDe(rec).onmessage!({ data: 'nem json é' });
  });
