...
/**
 * Os dublês daqui formam uma árvore de verdade, com só o que o envelope usa.
 * Os `as unknown as` estão todos na fronteira com `document`/`window`, cada um
 * comentado onde está: um dublê de `Document` completo não caberia no arquivo.
 */
interface NoFalso {
  tagName: string;
  children: NoFalso[];
...
// A fronteira com o navegador: o envelope toca `head.appendChild`,
// `createElement` e `querySelector`, e nada mais.
globalThis.document = {
  head: { appendChild: (node: NoFalso) => injected.push(node) },
  createElement: (tag: string) => makeNode(tag),
  querySelector: (selector: string) =>
    injected.find((node) => node.src && selector.includes(node.src)) || null,
} as unknown as Document;
globalThis.window = {} as unknown as Window & typeof globalThis;

/** Quantos `<iframe>` existem na subárvore — a asserção central do áudio órfão. */
function countIframes(node: NoFalso | null): number {
  if (!node) return 0;
  const self = node.tagName === 'IFRAME' ? 1 : 0;
  return node.children.reduce((total: number, child) => total + countIframes(child), self);
}
...
/** Uma chamada registrada no dublê do player: o nome, ou o nome mais argumentos. */
type ChamadaDoPlayer = string | [string, ...unknown[]];
...
function fakeYT({ autoReady = true } = {}) {
  ...
  } = { PlayerState: { ENDED: 0, PLAYING: 1, BUFFERING: 3 }, players: [], autoReady };

  class FakePlayer {
    mount: NoFalso;
    options: OpcoesDoPlayer;
    calls: ChamadaDoPlayer[];
    state: number;
    duration: number;
    title: string;
    currentTime: number;
    ...
    constructor(mount: NoFalso, options: OpcoesDoPlayer) {
      ...
      // O `createElement` dublê devolve um `NoFalso`; a assinatura do `Document`
      // promete um `HTMLIFrameElement`.
     ...
     */
    loadVideoById({ videoId, startSeconds }: { videoId: string; startSeconds?: number }) {
...
/** O módulo sob teste. O `?n=` obriga o Node a instanciá-lo de novo. */
type ModuloYouTube = typeof import('../src/lib/youtubePlayer.js');

let moduleCount = 0;
function freshModule(): Promise<ModuloYouTube> {
  moduleCount += 1;
  return import(`../src/lib/youtubePlayer.js?n=${moduleCount}`) as Promise<ModuloYouTube>;
}

/**
 * O namespace dublê entregue ao `window`: o envelope usa `Player` e três dos
 * seis `PlayerState`, e é só isso que o dublê monta.
 */
const comoYT = (YT: ReturnType<typeof fakeYT>): YTNamespace => YT as unknown as YTNamespace;

/**
 * Um `fetch` dublê. O envelope lê `ok` e `json()` da resposta e nada mais —
 * montar uma `Response` de verdade só para isso não provaria nada a mais.
 */
const comoFetch = (impl: (...args: never[]) => unknown): typeof fetch =>
  impl as unknown as typeof fetch;

/** Um `window` recém-zerado, para o caso que exercita o carregamento da API. */
const janelaLimpa = () => ({}) as unknown as Window & typeof globalThis;

function makePlayer(Ctor: ModuloYouTube['YouTubeTrackPlayer']) {
  ...
  } = { ended: [], errors: [], durations: [], titles: [] };
  const player = new Ctor({
    // O host é o nó dublê: é ele que o envelope varre no teardown.
    host: host as unknown as HTMLElement,
    onEnded: (videoId: string) => events.ended.push(videoId),
    onError: (payload: unknown) => events.errors.push(payload),
    onDurationKnown: (videoId: string, value: number) => events.durations.push([videoId, value]),
    onTitle: (videoId: string, title: string) => events.titles.push([videoId, title]),
  });
  return { player, events, host };
}

/** Nenhuma faixa deste arquivo pode ter passado pelo caminho de reuso. */
function assertNoReuse(YT: ReturnType<typeof fakeYT>) {
...
test('API já presente na página resolve sem injetar script nenhum', async () => {
  injected.length = 0;
  const YT = fakeYT();
  globalThis.window = { YT: comoYT(YT) } as unknown as Window & typeof globalThis;
  const { loadYouTubeApi } = await freshModule();
...
test('carregar a faixa anuncia duração e título e não sai tocando sozinha', async () => {
  ...
  assert.equal((inner.options.playerVars as Record<string, unknown>).playsinline, 1);
  assert.equal((inner.options.playerVars as Record<string, unknown>).controls, 0);
  assert.equal((inner.options.playerVars as Record<string, unknown>).start, 30);
...
test('AC1. a faixa seguinte constrói um player novo, destrói o anterior e deixa um iframe só', async () => {
  ...
  assert.equal((YT.players[1]!.options.playerVars as Record<string, unknown>).start, 12);
...
test('AC9. oEmbed respondendo devolve o título do vídeo', async () => {
  ...
  const fetchImpl = comoFetch(async (url: string, options?: unknown) => {
  ...
  const [url, options] = seen[0]! as [string, { credentials?: string; referrerPolicy?: string }];
...
test('AC10. rede caída, resposta não-ok, JSON inválido ou título vazio devolvem null', async () => {
  ...
  for (const [label, fetchImpl] of Object.entries(cases)) {
    assert.equal(await fetchYouTubeTitle('dQw4w9WgXcQ', { fetchImpl: comoFetch(fetchImpl) }), null, label);
  }
});
...
test('AC10. cancelar pelo signal devolve null sem lançar', async () => {
  ...
  const fetchImpl = comoFetch((_url: string, { signal }: { signal: AbortSignal }) =>
...
test('sem fetch no ambiente o título simplesmente não vem', async () => {
  ...
  // O cast: `fetch` é obrigatório no tipo do `globalThis`, e tirá-lo é o ponto.
...
test('AC1/AC2. 404 é vídeo indisponível; 401 e 403 são incorporação bloqueada', async () => {
  ...
  const responde = (status: number) =>
    comoFetch(async () => ({ ok: false, status, json: async () => ({ title: 'não devia ser lido' }) }));

  for (const [status, availability] of [[404, 'not-found'], [401, 'embed-blocked'], [403, 'embed-blocked']] as [number, string][]) {
...
test('AC5. o que não é prova de indisponibilidade vira unknown, e unknown enfileira', async () => {
  ...
  for (const [label, fetchImpl] of Object.entries(casos)) {
    const meta = await fetchYouTubeOEmbed('dQw4w9WgXcQ', { fetchImpl: comoFetch(fetchImpl) });
...
test('AC8/AC9. 200 devolve o título e o veredito ok — inclusive sem título legível', async () => {
  const { fetchYouTubeOEmbed } = await freshModule();
  const corpo = (json: unknown) => comoFetch(async () => ({ ok: true, status: 200, json: async () => json }));
...
test('AC10. uma requisição por link, e nenhuma quando não há o que perguntar', async () => {
  ...
  const fetchImpl = comoFetch(async (url: string) => {
    chamadas.push(url);
...
test('AC11. fetchYouTubeTitle continua sendo só o título, e continua não lançando', async () => {
  const { fetchYouTubeTitle } = await freshModule();
  const responde = (resposta: unknown) => comoFetch(async () => resposta);
...
test('AC6. sem entryId a decisão é só avisar, e o contador da faixa corrente fica intacto', async () => {
  ...
  // O cast do `42`: o caso é justamente um `entryId` de tipo errado.
  for (const entryId of [null, undefined, '', 42] as (string | null | undefined)[]) {
...
test('AC8. cinco mensagens distintas em pt-BR, com o título quando ele existe', async () => {
  const { planYouTubeError } = await freshModule();
  const notice = (code: number, extra: Record<string, unknown> = {}) =>
  ...
  // O cast do `42`: o caso é justamente um título de tipo errado.
  for (const title of [null, undefined, '', '   ', 42] as (string | null | undefined)[]) {
...
test('AC9. playerVars leva enablejsapi e a origem da página quando ela existe', async () => {
  ...
  globalThis.window = {
    YT: comoYT(YT),
    location: { origin: 'https://meet.exemplo.br' },
  ...
  const playerVars = YT.players[0]!.options.playerVars as Record<string, unknown>;
...
test('AC9. sem origem http(s) de verdade a chave origin nem entra — e nada estoura', async () => {
  ...
  /** Cada caso monta o `window` que o envelope vai enxergar. */
  const semLocation = () => ({ YT: fakeYT() });
  const casos: [string, { YT: ReturnType<typeof fakeYT>; location?: { origin: unknown } }][] = [
  ...
  for (const [label, fakeWindow] of casos) {
    ...
    const playerVars = fakeWindow.YT.players[0]!.options.playerVars as Record<string, unknown>;
