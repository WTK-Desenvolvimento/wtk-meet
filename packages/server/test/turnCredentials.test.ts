...
/** Um corpo de resposta do endpoint: o teste o lê campo a campo. */
type Corpo = Record<string, unknown>;

/**
 * Um `fetch` dublê. `fetchCloudflareIceServers` lê `ok` e `json()` da resposta
 * e nada mais — montar uma `Response` inteira não provaria nada a mais.
 */
...
function collectWarnings() {
  const lines: string[] = [];
  return { warn: (...args: unknown[]) => lines.push(args.join(' ')), lines };
}
...
test('sem as variáveis de ambiente devolve null — e não lança nem chama a rede', async () => {
  let chamou = false;
  const fetchImpl = comoFetch(async () => {
    chamou = true;
  });
...
test('sucesso devolve { iceServers, ttl, expiresAt } coerentes entre si (A1)', async () => {
  ...
  const fetchImpl = comoFetch(async (_url: string, options: { body: string }) => {
...
test('CF_TURN_TTL ausente manda 3600 para a Cloudflare (A6)', async () => {
  ...
  await fetchCloudflareIceServers({
    ...
    fetchImpl: comoFetch(async (_url: string, options: { body: string }) => {
...
test('resposta não-OK, corpo ilegível e lista vazia lançam (viram 502)', async () => {
  ...
  await assert.rejects(
    fetchCloudflareIceServers({ env, fetchImpl: comoFetch(async () => ({ ok: false, status: 401 })) }),
    /401/,
  );
  ...
  await assert.rejects(
    fetchCloudflareIceServers({
      env,
      fetchImpl: comoFetch(async () => ({ ok: true, json: async () => ({ iceServers: [] }) })),
    }),
    /vazio/,
  );
});
...
test('upstream que não responde é cortado pelo timeout, em tempo limitado (A3)', async () => {
  ...
  const fetchImpl = comoFetch((_url: string, options: { signal: AbortSignal }) =>
...
/**
 * Sobe o `index.ts` real num processo filho, com a Cloudflare dublada, e devolve
...
test('com as variáveis e a Cloudflare boa: 200 com ttl e expiresAt (A1)', async (t) => {
  ...
  assert.ok(Number.isFinite(Date.parse(String(body.expiresAt))));
});
...
test('Cloudflare não-OK, lançando, vazia ou muda: 502 turn-upstream (A3)', async (t) => {
  for (const stub of ['error', 'throw', 'empty']) {
    ...
    assert.equal(((await res.json()) as Corpo).error, 'turn-upstream', `stub=${stub}`);
  }
