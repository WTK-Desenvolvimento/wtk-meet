 ...
 * trafega no fio.
 *
 * Complementa, e não substitui, `client/test/joinRequestSignaling.test.ts`:
...
function freePort(): Promise<number> {
  return new Promise<number>((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      // O `as`: `listen` já respondeu, e um servidor TCP escutando tem endereço
      // com porta — o `string | null` da assinatura cobre socket unix e servidor fechado.
      const { port: chosen } = probe.address() as AddressInfo;
...
function once(socket: Socket, event: string, timeout = EVENT_TIMEOUT): Promise<Payload> {
 ...
 */
function nothing(socket: Socket, event: string, timeout = 300): Promise<void> {
...
async function enterEmptyRoom(roomId: unknown, displayName?: unknown) {
...
async function enterWithApproval(roomId: string, displayName: string, approver: Socket) {
  const socket = await connect();
...
test('/rooms/:id/occupancy responde só { occupied } — sem nomes e sem contagem', async () => {
  ...
  const depois = await (async () => {
    for (let i = 0; i < 20; i += 1) {
      const body = (await (await fetch(`http://127.0.0.1:${port}/rooms/${room}/occupancy`)).json()) as Payload;
